...
; 0x900000    - Stack for 32/64-bit modes (grows downward) - AFTER BSS SECTION!
...
PAGE_TABLE_BASE       equ 0x820000      ; CRITICAL: AFTER BSS (BSS ends at 0x810198)
...
STACK_BASE            equ 0x900000      ; CRITICAL: Stack AFTER BSS section (BSS ends ~0x810000)
...
setup_paging_fixed:
    ...
    ; PD (0x502000) - маппинг первых 128MB как 2MB страницы (для динамического kernel heap)
    ; CRITICAL: Need to map enough memory for kernel heap allocation (up to 16MB)
    ; PMM can allocate anywhere in first 128MB, so we map it all
    mov edi, PAGE_TABLE_BASE
    add edi, 0x2000       ; PD offset
    mov eax, 0x000083     ; Present, Writable, Page Size (2MB)
    mov ecx, 64           ; 64 записей по 2MB = 128MB
